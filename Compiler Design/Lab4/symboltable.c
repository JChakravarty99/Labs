...
const char *keywords[]={"auto", "double", "int", "struct", "break", "else", "long", "switch", "case", "enum", 
						"register", "typedef", "char", "extern", "return", "union", "continue", "for", "signed",
						"void", "do", "if", "static", "while", "default", "goto", "sizeof", "volatile", "const",
						"float", "short", "unsigned", "printf", "scanf", "true", "false"};
const char *datypes[]={"int","char","void","float","bool"};

//function to check if entered string is a datatype
int isdtype(char *w)
{
	int i;
...
//function to check if current string is keyword
...
//defining structure for symbol table
struct sttable
{
	int sno;
	char lexeme[128];
...
//function to find the co0rrect tacle for given token 
...
//function to make proper symbol table entry for each token
...
//function to create token and populate token structure
...
//function to determine size or number of bytes occupired by an identifier
int sz(char *w)
{
	if(strcmp(w,"int")==0)
		return 4;
...
int main()
{
    ...
    scanf("%s", filename);
    fa = fopen(filename, "r");
    //checking file pointer integrity
    if (fa == NULL)
    {
    ...
    //This section of code works to remove white spaces, single and multiline comments from the input code.
    ca = getc(fa);
	while (ca != EOF){
		if(ca==' ')
		{
	...
	//Re-opening previous output file as input file 
	fa = fopen(filename, "r");
	if(fa == NULL)
	{
		printf("Cannot open input file!");
	...
	//creating a temp file to store output
	fb = fopen("temp.c", "w+");
	if(fb == NULL)
	{
	...
	//This section of code remover pre-processor directives
	ca = getc(fa);
	while (ca != EOF)
    {
        if(ca=='"')
	...
	//reopeniong temp.c as input file and overwriting output file to update it to latest code
	fa = fopen("temp.c", "r");
	fb = fopen(filename, "w");
	ca = getc(fa);
	...
	//Further declarations to proceed to token generation and symbol table creation phase
	struct token tkn;
	...
	//generating tokens and symbol table. 
